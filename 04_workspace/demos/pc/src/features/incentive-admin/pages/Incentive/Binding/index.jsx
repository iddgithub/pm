...
export default function Binding() {
  ...
  const summary = useMemo(() => ({
    ...
    pendingCount: filteredData.filter((item) => item.bindingStatus === '待确认').length,
  }), [filteredData])
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 100,
      render: (value) => <Tag color={value === '已绑定' ? 'blue' : 'orange'}>{value}</Tag>,
    },
    {
      title: '操作',
      key: 'action',
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: 16, gap: 16, overflow: 'hidden' }}>
      ...
      <Row gutter={16}>
        ...
        <Col span={8}><Card><Statistic title="待确认绑定" value={summary.pendingCount} suffix="条" valueStyle={{ color: '#FF6B00' }} /></Card></Col>
      </Row>
